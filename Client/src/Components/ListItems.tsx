...
React.useEffect(()=>{
  axios.get(API+`favourite_groups/${parsed_data?.sendeddata?.userId}`).then((data)=>{
...
const GroupAddToFavourite =async (e:React.MouseEvent<SVGSVGElement, MouseEvent>,groupId:number)=>{
           e.preventDefault();
           e.stopPropagation();
           await axios.post(API+`profiles_add_to_groups`,{groupId,userId:parsed_data?.sendeddata?.userId}).then(async (data)=>{
            console.log(data.data);
           await axios.get(API+`favourite_groups/${parsed_data?.sendeddata?.userId}`).then((data)=>{
                console.log(data.data,'favourite_groups');
                setFavouriteGroup(data.data);
              }).catch((error)=>{
                console.log(error.message)
...
{(allgroupsofuser !== null) && allgroupsofuser.map((userdata, index) => (
    <React.Fragment key={index}>
        <ListItem alignItems="flex-start" onClick={(e)=>FetchGroupData(e,userdata?.groupId)}>
            ...
            />
     <ListItemAvatar>
     {Array.isArray(favouriteGroup) && favouriteGroup.flat().includes(userdata?.groupId) ?  
    <StarIcon sx={{opacity:0.8,color:"yellow"}}/>:  
    <StarOutlineIcon  onClick={(e) => GroupAddToFavourite(e, userdata.groupId)} sx={{opacity:0.8}}/>}
