...
export default function BasicMenu({receiverId,senderId,setLoaddata,setChatting}:
  {receiverId:string,senderId:string,setLoaddata:(data:boolean) => void,setChatting:(data:any) => void}) {
...
if (typeof imagefile === 'string') {
  ...
  try {
    setLoadImage(true);
      const response = await axios.post("https://api.cloudinary.com/v1_1/dvbvggl5f/upload", formData);
    axios.post(API+"sendmessage",{SenderId:senderId,ReceiverId:receiverId,message:response.data.url}).
    then((data)=>{console.log(data.data);setOpened(false);setLoaddata(true);setLoadImage(false);setAnchorEl(null)}).catch((error)=>console.log(error.message))
  } catch (error) {
      console.error(error);
  }
...
try {
      ...
      // Upload file
      const uploadResponse = await axios.post('http://localhost:5001/uploads', formData).
      then(async (data)=>{data.data && 
        await axios.post(API+"sendmessage",{SenderId:senderId,ReceiverId:receiverId,message:data.data}).
      then((data)=>{console.log(data.data);setOpened(false);setLoaddata(true);setLoadImage(false);setAnchorEl(null)}).
 ...
 <input
      ...
      <label htmlFor="imageUploadInput" style={{ display: 'inline-block' }}>
        <MenuItem>Image<PhotoOutlinedIcon sx={{marginLeft:"10px"}}/></MenuItem>
      </label> <br />
      ...
      <label htmlFor="videoUploadInput" style={{ display: 'inline-block' }}>
        <MenuItem>Video<VideoCameraBackOutlinedIcon sx={{marginLeft:"10px"}}/></MenuItem>
      </label> <br />
      <input
        type="file"
        accept="file/*"
        ...
        id="fileUploadInput"
      />
      <label htmlFor="fileUploadInput" style={{ display: 'inline-block' }} aria-disabled>
        <MenuItem>Files<InsertDriveFileOutlinedIcon sx={{marginLeft:"15px"}}/></MenuItem>
      </label>
          </Menu>
...
<object width='100%' height='600px' data={image ? image.toString():''} type='application/pdf'></object>}
       </DialogContent>
